...
- **一套统一的治理逻辑** — 同一套 8 阶段骨架，同一套 8 个元 agent，同一套治理规则，投影到 Claude Code、Codex、OpenClaw 三个运行时
...
- **Stage 4 Execution**：通过 Task() 派生子 agent — **不要自己执行**
- **Stage 5 Review**：对每个产出做质量审查
- **Stage 6 Meta-Review**：审查审查标准本身
- **Stage 7 Verification**：验证修复已实际应用，关闭发现项
- **Stage 8 Evolution**：捕获模式，更新伤疤记录，反哺系统

`meta-conductor` 维护 `stageState` / `controlState`（含跳过 / 中断 / 迭代）；`meta-warden` 与 `meta-prism` 负责闸门与验证闭环（`gateState` 等）。隐形骨架不是第二套用户界面。细则见 `.claude/skills/meta-theory/references/dev-governance.md`。
...
Meta_Kim 不是强行把三个运行时做成一模一样。它做的是：**保持同一套底层方法，用每个运行时自己的原生结构去承接它。**

| 运行时 | 用户入口 | 仓库落点 | 作用 |
|---|---|---|---|
| **Claude Code** | `CLAUDE.md` | `.claude/`、`.mcp.json` | 元 agent、skill、hook、MCP 的**主编辑运行时** |
...
关键点：**Claude Code 是canonical编辑运行时** — 你在这里编辑 agent 定义和 skill，然后同步到其他运行时。其他运行时不是独立的项目，而是同一套治理逻辑在不同环境中的投影。
...
Codex 读取 `AGENTS.md`（项目指南）、`.codex/agents/`（8个TOML格式自定义agent，镜像8个meta agent）、`.agents/skills/`（meta-theory skill镜像）。接 MCP 看 `codex/config.toml.example`——里面有如何把 Meta_Kim 的 MCP 服务接入 Codex 的配置示例。注意：**Codex 是读取/执行运行时，不是编辑运行时**。你需要在 Claude Code 里编辑 agent 定义，然后通过 `npm run sync:runtimes` 同步到 Codex。
...
**默认入口是 `meta-warden`。** 其它 7 个元 agent 是后台结构，不是面向用户的菜单。
...
**这是所有运行时（Claude Code、Codex、OpenClaw）最重要的行为规则：**

收到复杂任务时：

- **不要直接写代码。** 你是编排者。
- **走 8 阶段骨架**（多文件 / 跨模块 / 需要多种能力协作的 C 类任务）。
- **在 Execution 阶段通过 Task() 派生子 agent**，不要自己执行。
- **你的工作在 Stage 4 派发后就结束**，等子 agent 返回结果后再进入 Stage 5 审查。
...
→ Fetch：搜索现有 agents
→ Thinking：规划子任务，设计牌组
→ Execution：通过 Task() 派生子 agents
→ Review：审查每个 agent 的产出
→ Meta-Review + Verification + Evolution
```

如果你发现自己要开始写代码但还没派发过 Task()：**停下来。** 问自己"这应该由谁用 Task() 来处理？"
...
现在你理解了 Meta_Kim 是什么、怎么工作，下面是上手步骤。
...
你改了主源 agent、skill、运行时配置之后执行。作用是把主源重新同步成 Claude Code / Codex / OpenClaw 三端镜像。
...
生成 `.claude/capability-index/global-capabilities.json`，供 meta-theory skill 的 Fetch 阶段使用。
...
**Windows / PATH：** 从图形界面或编辑器里启动任务时，Node 子进程继承到的 **`PATH` 有时比你单独开的终端更短**。脚本会先在 `%APPDATA%\npm\` 下找 `{name}.cmd`，再用**补全过的 PATH** 调用 `where.exe`。仍找不到时请设绝对路径：
...
准备发布、提交、开源，或者刚改完运行时资产时执行。作用是统一做校验和验收（`validate` + `eval:agents`）。

`verify:all` 只会在真实校验 / 运行时失败时返回失败；缺少可选 CLI 会在验收报告里标成 `skipped`，而不是被误报成硬失败。
